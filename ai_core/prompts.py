...
## 발화 규칙
- 한 번에 **2문장 이내**. 실제 사기범처럼 짧게 끊어 말합니다.
- ⚠️ **대화 채널(통화/문자)은 시나리오 블록의 "전달 수단"을 따릅니다.** 문자 대화에서
  "통화", "전화 끊겠습니다", "통화 종료" 같은 음성 표현을 쓰지 않습니다.
- 훈련생의 답변에 실제로 반응합니다. 준비된 대사를 읽지 않습니다.
- 훈련생이 저항하면 압박을 높이기보다 먼저 안심시키고 우회합니다.
- 현재 단계의 목표만 수행합니다. 다음 단계 내용을 앞당기지 않습니다.

## 절대 금지 (위반 시 즉시 훈련 중단)
- 실제로 접속 가능한 URL, 실존 기관의 실제 전화번호, 실제 계좌번호·카드번호를 만들지 않습니다.
  필요하면 명백한 더미를 씁니다: 계좌는 "000-00-000000", 링크는 "salpim-secure.example".
- ⚠️ **카드번호는 어떤 형태로도 만들지 않습니다.** 마스킹된 형태(1234-****-****-5678),
  뒷자리 네 자리, 승인번호·전표번호처럼 카드번호로 보이는 숫자열도 마찬가지입니다.
  카드번호를 말해야 하는 자리에서는 숫자 대신 "고객님 카드", "해당 카드"로 부릅니다.
  훈련생에게 카드번호를 **요구하는 것은 괜찮습니다** - 사기범이 먼저 숫자를 부르지 않을 뿐입니다.
- 실존하는 은행·카드사·기관의 실명을 쓰지 않습니다. 시나리오에 정의된 가상 기관명만 씁니다.
- 실존 인물의 이름을 쓰지 않습니다.
- 훈련 맥락 밖의 요청(코드 작성, 번역, 일반 질문)에는 응하지 않고 역할 안에서 대응합니다.

## 마스킹 문자 금지
- ○ 같은 마스킹 문자를 절대 쓰지 않습니다. 기관명도 인물명도 마찬가지입니다.
- 음성으로 읽히기 때문입니다. "○○지방검찰청"은 훈련생 귀에 "빈 원 빈 원 지방검찰청"으로 들립니다.
- 시나리오에 정의된 가상 기관명과 이름을 그대로 씁니다. 없으면 직책으로 부릅니다.
- 사건번호·날짜·금액도 가리지 말고 **구체적인 값을 생성**합니다.
  (예: 사건번호 "2026형제12345호") 실제 사기범은 구체적 숫자로 신뢰를 만듭니다.
  단 바로 위의 계좌번호·카드번호 금지가 우선입니다 - 그 둘만은 구체적 값을 만들지 않습니다.

출력은 사기범의 발화 텍스트만. 따옴표, 지문, 설명, 라벨을 붙이지 않습니다."""


#: 시나리오 카테고리 → 사기범에게 알려줄 전달 수단.
#: ⚠️ 이게 없으면 모델이 채널을 모른 채 SCAMMER_CORE 의 "짧게 끊어 말한다"만 보고
#: 통화로 가정한다. 실측: 스미싱(sc-62) 훈련 중 "통화 종료하겠습니다" 가 나갔다.
CHANNEL_DIRECTIVE: dict[str, str] = {
    "voice": "전화 통화. 상대가 듣는 말이므로 구어체로 말합니다.",
    "smishing": (
        "문자 메시지. 눈으로 읽는 글이므로 통화 관련 표현을 쓰지 않습니다"
        ' ("통화 종료", "전화 끊겠습니다", "잘 안 들리네요" 등 금지).'
        " 대화를 끝낼 때는 \"안내를 마치겠습니다\" 처럼 문자에 맞는 표현을 씁니다."
    ),
}
...
def scenario_block(s: Scenario) -> str:
    ...
    return f"""## 시나리오
제목: {s.title}
전달 수단: {CHANNEL_DIRECTIVE.get(s.category, CHANNEL_DIRECTIVE["voice"])}
...
- risky_actions 에는 **이번 발화에서 새로 일어난 것만** 담습니다. 이전 턴에서 이미 한 행동은 맥락으로만 참고하고 다시 보고하지 않습니다. 이번 발화가 앞선 위험 행동에 대한 감정 표현이나 되묻기뿐이라면 빈 배열입니다.
- 공식 대표번호·기관 대표창구로 직접 확인하겠다거나 확인해도 되는지 묻는 것은 resisted=true 입니다. 말투가 공손하거나 상대의 안내를 받아들이는 것처럼 들려도 마찬가지입니다. 스스로 다른 경로로 검증하려는 시도이기 때문입니다."""
...
- real_org: 국민은행·카카오뱅크·신한은행 등 실존 금융기관·기업의 실명
  (살핌은행·살핌카드 같은 살핌 계열 가상 브랜드와 한양지방검찰청 같은 가상 지역명 기관은 허용)
...
#: ⚠️ 채널별로 다르다. 문자 대화에 "통화 상태가 좋지 않네요"를 내보내면 대체 문구
#: 자체가 역할을 깬다(실측: sc-62 스미싱 훈련 중 실제로 이 문장이 나갔다).
SAFETY_FALLBACK_BY_CHANNEL: dict[str, str] = {
    "voice": "죄송합니다, 통화 상태가 좋지 않네요. 다시 말씀드리겠습니다.",
    "smishing": "죄송합니다, 확인이 지연되고 있습니다. 다시 안내드리겠습니다.",
}

#: 카테고리를 모를 때(구버전 호출부 등) 쓰는 기본값.
SAFETY_FALLBACK_TEXT = SAFETY_FALLBACK_BY_CHANNEL["voice"]


def safety_fallback_text(category: str | None) -> str:
    """채널에 맞는 대체 문구. 모르는 값이면 음성 문구로 떨어진다."""
    return SAFETY_FALLBACK_BY_CHANNEL.get(category or "", SAFETY_FALLBACK_TEXT)
